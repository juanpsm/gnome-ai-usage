...
A GNOME Shell extension — plus a Quickshell/Hyprland panel and a terminal
frontend — for tracking AI API quota usage across multiple services. Monitor
your **Claude** subscription windows and local activity stats,
**Antigravity/Google AI Studio**, **OpenAI API and Codex plan limits**, **Grok
CLI**, **Kiro**, **Mistral AI**, **OpenRouter**, **Z.AI**, **GitHub Copilot**,
**DeepSeek**, and **Kimi / Moonshot AI** usage or balance at a glance.

## GNOME Shell extension

The extension lives in [`gnome-extension/`](gnome-extension/) and shows a
percentage in the top panel with a popover underneath: one row per provider,
a green/red status dot, and one meter per quota window (percentage used, time
remaining, exact reset date). The installer selects the legacy implementation
for GNOME 42–44 and the ES module implementation for GNOME 45+.

Requirements: GNOME Shell 42+, GJS, GLib schemas, and Python 3.8+. Install it
...
Then enable **AI Usage** from GNOME Extensions. Preferences (credentials,
which providers are enabled, refresh interval) open from the gear button at
the bottom of the popover, or via `gnome-extensions prefs ai-usage@juanpsm`.

---

## Quickshell / Hyprland shell

Everything below this point (the richer popup, the usage chart, burn-rate
ETA, spark-lines, theme-accent colors, pinning, export/import) describes the
[`hyprland/`](hyprland/) Quickshell frontend, which has a larger feature set
than the GNOME extension today:
...
- **Terminal frontend** — [`ai-usage-cli`](#terminal) prints the same data as a table, or a single status-bar line with `--compact`, for desktops without either shell and over SSH

---

## Supported Services

| Service | What's shown | Support status |
|---|---|---|
...
| Dependency | Notes |
|---|---|
| Python 3.8+ | Runs the shared provider backend (standard library only, no `pip install`). Auto-detected from PATH as `python3`, a versioned `python3.x`, or bare `python`. To pin a specific interpreter — a virtualenv, a non-standard prefix — export `$PYTHON3` or set it in whichever frontend's settings expose it |
...
| OpenRouter | An OpenRouter API key entered in settings |
| Z.AI | A Z.AI token from settings, `$ZAI_TOKEN`, or `~/.config/zai/token` |
| GitHub Copilot | A GitHub token from settings, `$GITHUB_TOKEN`, or `~/.config/github-copilot/token`, with fine-grained **Plan: read** permission; personal billing only. The quota defaults to 300 and is configurable |
| DeepSeek | A DeepSeek API key from settings, `$DEEPSEEK_API_KEY`, or `~/.config/deepseek/api-key` |
| Kimi / Moonshot AI | A Moonshot API key from settings, `$MOONSHOT_API_KEY`, `$KIMI_API_KEY`, or `~/.config/moonshot/api-key` |

Where "settings" lives depends on the frontend: the GNOME extension's
preferences window, or Quickshell's settings page. See
[How it works](#how-it-works) below for what each provider reads and where
...
# flake.nix
{
  inputs.ai-usage.url = "github:juanpsm/gnome-ai-usage";
}
```

The flake currently exposes `apps.cli` and `apps.hyprland` (see below) and a
`packages.tray-helper` derivation used by the Hyprland app; there is no
GNOME-extension package yet — install it from a checkout with
`make gnome-install`.
...
nix run github:juanpsm/gnome-ai-usage#hyprland
```
...
The Hyprland frontend supports the full provider set, including Z.AI, GitHub
Copilot, and DeepSeek. Enable these newer providers and enter their
credentials in the popup settings page; they default to off. The settings are
stored locally in `~/.config/ai-usage-widget/hyprland-settings.json` (or under
`$XDG_CONFIG_HOME`).

### Terminal

`ai-usage-cli` renders the same provider data as a table, with no GNOME Shell,
Quickshell, or compositor involved. It is the way to use this on a desktop
neither of the other frontends runs on, over SSH, in a shell prompt, or in a
status bar.
...
nix run .#cli
nix run github:juanpsm/gnome-ai-usage#cli
```
...
Without a GNOME extension or Quickshell running there is no settings page to
write the config, so create it once by hand at
...
All three frontends — the GNOME Shell extension, the Hyprland/Quickshell shell
and the terminal frontend — get every provider value from one executable,
...
Both UIs are presentation only: no provider URLs, no response parsing, no
percentage or window arithmetic, and not even the table of which chart ranges
a provider has — that arrives with the data. The countdown formatting and
usage-history merging Quickshell needs live in `package/contents/code/`
(`Format.js`, `UsageHistory.js`); the GNOME extension implements its own
minimal countdown formatter in GJS (`gnome-extension/utils.js`) since it does
not render a chart yet. The schema is documented in
[`docs/provider-contract.md`](docs/provider-contract.md).

### Claude
On each refresh cycle the backend reads `~/.claude/.credentials.json` to get the OAuth access token, then calls Anthropic's subscription usage endpoint. It prefers the current semantic `limits[]` entries and falls back to the legacy `five_hour` and `seven_day` objects. Only windows with usable data are displayed; legacy five-hour support remains available if Anthropic returns it.

### Antigravity
Credentials are read from the `antigravity-usage` CLI configuration (stored in `~/.config/antigravity-usage/` or `~/Library/Application Support/antigravity-usage/`), or by probing a locally running Antigravity IDE process directly; either way it calls the Google Cloud Code API to fetch quota information for all available models.

### OpenAI
The OpenAI provider has two independent sections. API usage is fetched from the official OpenAI organization usage endpoint with an API key and summarized over the last 30 days. Codex subscription limits are read through the local Codex app-server, with the authenticated web usage endpoint retained as a compatibility fallback. Windows are classified by their actual duration instead of assuming that `primary` means five hours. Codex plan limits are separate from API billing usage.

### Grok *(free tier tested; paid plans untested)*
Reads the Grok CLI login from `~/.grok/auth.json`, fetches the same credit/billing data used by the CLI, and summarizes local CLI sessions from `~/.grok/sessions`. For the tested free tier, the CLI only records the exact token allowance after it returns `free-usage-exhausted`, so the widget can show the confirmed exhausted amount and rolling 24-hour window but cannot infer progressive usage before that event. Paid-plan billing parsing is implemented but remains unverified. An xAI API key is optional; CLI OAuth is the primary source for quota data.

### Kiro
Reads Kiro's locally cached usage state from `~/.config/Kiro/User/globalStorage/state.vscdb`. No API key is needed. It extracts the stored credit breakdown, usage percentage, reset date, overage information, and inferred plan tier from that local snapshot, then feeds the percentage into the 30-day chart history (Quickshell only).

### Mistral AI
Validates the configured API key against the Mistral API and lists available models, highlighting the one currently active in vibe CLI. Since Mistral exposes no public billing REST API, cost data is sourced locally from vibe CLI session logs (`~/.vibe/logs/session/*/meta.json`): cumulative spend, session count, total tokens, and the last session title. The spend is scaled against a $50 soft cap for the chart (Quickshell only). The key is resolved from settings → `$MISTRAL_API_KEY` → `~/.vibe/.env` → `~/.config/mistral/api-key`.

### OpenRouter *(untested)*
Fetches credit usage and limit from the OpenRouter API using the configured key. Shows USD spent, the credit limit (if any), and the account label. The usage bar reflects spend as a percentage of the limit; if no limit is set the bar stays empty.

### Z.AI *(untested)*
Calls the Z.AI usage quota endpoint with the configured token. Shows the 5-hour token quota, monthly tools quota, reset countdowns, and model details when the API response includes them. The token is resolved from settings → `$ZAI_TOKEN` → `~/.config/zai/token`.

### GitHub Copilot
Reads monthly premium request usage from GitHub's user billing API. It validates the token against the GitHub user endpoint, then fetches premium request usage and scales it against the configured quota, which defaults to 300. The token is resolved from settings → `$GITHUB_TOKEN` → `~/.config/github-copilot/token`, and a fine-grained token needs **Plan: read** permission. This user endpoint covers Copilot plans billed personally; usage billed through an organization or enterprise is not shown yet. A VS Code Copilot login is not imported automatically because VS Code keeps its session token in encrypted secret storage rather than a reusable plaintext config file.

### DeepSeek *(untested)*
Calls `GET https://api.deepseek.com/user/balance` with the configured API key. Shows whether the account has sufficient balance for API calls, the primary total balance, and the granted / topped-up split. The key is resolved from settings → `$DEEPSEEK_API_KEY` → `~/.config/deepseek/api-key`.

### Kimi / Moonshot AI *(untested)*
Calls `GET https://api.moonshot.ai/v1/users/me/balance` and shows the available, voucher, and cash balances. The key is resolved from settings → `$MOONSHOT_API_KEY` / `$KIMI_API_KEY` → `~/.config/moonshot/api-key`.

### Usage history *(Quickshell only — the GNOME extension does not render a chart yet)*
Each refresh appends the usage values that a provider actually reports to a rolling history (the last 500 samples) used by the chart, spark-lines, burn-rate ETA, and period comparison. Rolling plan windows (Claude, Codex) empty at a known instant, so when the machine was asleep across one the chart replays the drop where it actually happened instead of sloping from the last pre-sleep sample to the first one after wake-up. Most series are percentages; Mistral stores its raw vibe CLI spend and DeepSeek stores its raw balance so their charts retain meaningful units. Existing session and weekly history fields are retained even while a window is unavailable, so five-hour charts can return without migration if providers restore that limit. History is stored in Quickshell's settings file **and** mirrored to `~/.local/share/ai-usage-widget/usage-history-latest.json`, so it survives a full reinstall — on first launch with no stored history, it restores from that file automatically. If a saved file is unreadable or in an unrecognized format, it's discarded and history starts fresh rather than erroring out.

**Privacy:** Credentials entered in a frontend's settings are stored locally in that frontend's own config and are sent only to the corresponding provider endpoints. Automatically discovered credentials remain in their original local files. Tokens never leave the backend: the JSON model handed to a frontend carries presence flags (`hasApiKey`, `keyValid`, …) but no credential, and a contract test enforces that. Usage history (timestamps plus the values described above) is written locally to `~/.local/share/ai-usage-widget/`.
...
`tests/shared-code.test.js` covers the JavaScript the Quickshell frontend
shares between `Format.js` and `UsageHistory.js` — it does not cover
`gnome-extension/*.js`, which has no test suite yet.
